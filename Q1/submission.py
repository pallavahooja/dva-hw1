...
class Graph:
    ...
    def add_node(self, id: str, name: str) -> None:
        ...
        if (id, name) not in self.nodes:
            self.nodes.append((id, name.replace(',', '')))
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for edge in self.edges:
            for node_id in edge:
                if node_id in degree_counts:
                    degree_counts[node_id] += 1
                else:
                    degree_counts[node_id] = 1
        ...
        max_degree = max(v)
        return { k[v.index(max_degree)]: max_degree }
    ...
    def num_nodes_with_multiple_degrees(self)->int:
      degree_counts = {}
      for edge in self.edges:
          for node_id in edge:
              if node_id in degree_counts:
      ...
      return len([v for v in list(degree_counts.values()) if v > 1])
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#4224fc11-bda3-41bd-b0d9-929029b8b654"
...
if __name__ == "__main__":
    ...
    # for i in range(2):
    #     if i == 0:
    #         nodes = tmdb_api_utils.add_coactors(startNode)
    #         new_nodes = []
    #     else:
    #         nodes = new_nodes
    #     for node in nodes:
    #         existing_ids = [node[0] for node in graph.get_nodes()]
    #         added = tmdb_api_utils.add_coactors(node, existing_ids)
    #         if i == 0:
    #             new_nodes.extend(added)

    # print('Total nodes:' + str(graph.total_nodes()))
    # print('Total edges:' + str(graph.total_edges()))
    ...
    print('k: ' + str(graph.num_nodes_with_multiple_degrees()))
